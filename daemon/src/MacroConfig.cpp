...
void InputMapper::initializeAppMacros() {
  ...
  defaultMacros.push_back(
      KeyAction{KeyTrigger{{{EV_KEY, BTN_FORWARD, KEY_PRESS, WITHHOLD_NO,
  ...
  defaultMacros.push_back(KeyAction{
      KeyTrigger{
          {{EV_KEY, G6_VIRTUAL, KEY_PRESS, WITHHOLD_NO, KEY_REPEAT_BREAKS_NO}}},
  ...
  terminalSpecificMacros.push_back(KeyAction{
      KeyTrigger{
          {{EV_KEY, G1_VIRTUAL, KEY_PRESS, WITHHOLD_NO, KEY_REPEAT_BREAKS_NO}}},
      {{KEY_LEFTCTRL, 1},
       {KEY_LEFTALT, 1},
       {KEY_C, 1},
       {KEY_C, 0},
  ...
  //       KEY_REPEAT_BREAKS_NO}, {BTN_LEFT, KEY_PRESS, WITHHOLD_NO,
  //       KEY_REPEAT_BREAKS_NO}}},
  //                 {{KEY_LEFTCTRL, KEY_RELEASE},{KEY_7, KEY_PRESS}, {KEY_7,
  //                 KEY_RELEASE}}, // we must add the control up so that the 5
  //                 will print "Triggering Terminal Ctrl+Left Click macro",
  //                 []() {
  //                     executeBashCommand("sudo -u yaniv
  //                     DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus
  //                     DISPLAY=:0 notify-send \"hi\" \"2\"");
  //                 }});
//   terminalSpecificMacros.push_back(KeyAction{
//       KeyTrigger{
...
//                            "bus DISPLAY=:0 notify-send \"hi\" \"2\"");
//       }});
  //   terminalSpecificMacros.push_back(
  ...
  codeSpecificMacros.push_back(KeyAction{
      KeyTrigger{
  ...
  codeSpecificMacros.push_back(
      KeyAction{KeyTrigger{{{EV_KEY, BTN_FORWARD, KEY_PRESS, WITHHOLD_NO,
                             KEY_REPEAT_BREAKS_NO}}},
                {{KEY_A, 1}, {KEY_A, 0}},
                "Triggering mouse forward button (press) -> A in Code app "
                "(override default)",
                nullptr});
  ...
  chromeSpecificMacros.push_back(KeyAction{
      KeyTrigger{
          {{EV_KEY, KEY_LEFTCTRL, KEY_PRESS, WITHHOLD_NO,
            KEY_REPEAT_BREAKS_YES},
           {EV_KEY, KEY_V, KEY_PRESS, WITHHOLD_YES, KEY_REPEAT_BREAKS_NO}}},
