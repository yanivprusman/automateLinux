...
export default class ClockExtension extends Extension {
    ...
    async enable() {
        ...
        try {
            ...
            console.error(`[AutomateLinux] Error in enable(): ${e.message}`);
        }
    }
    ...
    async _onActiveWindowChanged() {
        try {
            const windowInfo = this._getActiveWindowInfo();
            this.logger.log(`Active window changed (proactive): ${JSON.stringify(windowInfo)}`);
            ...
            console.error(`[AutomateLinux] Error in _onActiveWindowChanged(): ${e.message}`);
        }
    }
}
